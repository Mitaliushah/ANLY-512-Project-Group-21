###########################################################################################################
# ANLY 512 Poster: Predictive Models with RFM Label
# Project Team : 21
...
################### READ DATASET ##########################################
ecommerce_data <- read.csv("features/features.csv", stringsAsFactors = TRUE)
str(ecommerce_data)
...
shuffled_data <- ecommerce_data[sample(1:nrow(ecommerce_data)), ]
...
# save the plot
tiff(filename = "ROC-RFM_new.png", width = 3500, height = 2000, res = 600)
...
write.csv(stacked, "RFM_new/RFMstacked_ensemble.csv", row.names = FALSE)
...
stacked <- cbind(actual, prediction_ensemble)
# save the predictions as a dataframe
write.csv(stacked, "RFM_new/gradient-boosting.csv", row.names = FALSE)
...
stacked <- cbind(actual, prediction_ensemble)
# save the predictions as a dataframe
write.csv(stacked, "RFM_new/random-forest.csv", row.names = FALSE)
...
stacked <- cbind(actual, prediction_ensemble)
# save the predictions as a dataframe
write.csv(stacked, "RFM_new/logisticRegression.csv", row.names = FALSE)
